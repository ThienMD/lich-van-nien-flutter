...
class DeepSeekService {
  ...
  Future<String> askHoroscope(HoroscopePromptVO prompt) async {
    if (!isConfigured) {
      return _buildPreviewResponse(prompt, includeSetupHint: true);
    }
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final List<dynamic> choices = payload['choices'] as List<dynamic>? ?? <dynamic>[];
      ...
      return _buildPreviewResponse(prompt, error: 'Không kết nối được tới DeepSeek ngay lúc này.');
    }
  }
